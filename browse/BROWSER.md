...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.avadbot/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `~/.claude/skills/browse/`. After making changes:

1. Copy the updated source: `cp -r avadbot/browse/* ~/.claude/skills/browse/`
2. Rebuild: `cd ~/.claude/skills/browse && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/browse/dist/browse`
