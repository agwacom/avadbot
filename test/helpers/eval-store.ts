/**
 * Eval result persistence and comparison for avadbot.
 *
 * EvalCollector accumulates test results, writes them to
 * ~/.avadbot-dev/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const DEFAULT_EVAL_DIR = path.join(os.homedir(), '.avadbot-dev', 'evals');
