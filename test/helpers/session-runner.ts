/**
 * Claude CLI subprocess runner for avadbot skill E2E testing.
 *
...
const AVADBOT_DEV_DIR = path.join(os.homedir(), '.avadbot-dev');
const HEARTBEAT_PATH = path.join(AVADBOT_DEV_DIR, 'e2e-live.json');
...
export async function runSkillTest(options: {
  ...
  if (runId) {
    try {
      runDir = path.join(AVADBOT_DEV_DIR, 'e2e-runs', runId);
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.avadbot', 'test-transcripts');
