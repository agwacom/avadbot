...
version: 2.9.0
description: |
  Start a new project the right way. Asks 5 product questions (problem, user,
  30-day goal, project type, stack), then generates 16 pre-filled files —
  not empty skeletons. PRD.md has your problem statement, README.md has your
  audience, TODOS.md has your milestone broken into tasks. Detects tech stack,
  creates GitHub repo, and chains into design consultation or CEO review.
  Use when starting a new project from scratch, scaffolding a project,
  or "init a project".
  Do NOT use for adding features to existing projects, creating skills
  (use /avad-add-skill), or upgrading from gstack (use /avad-gstack-upgrade).
allowed-tools:
  - Bash
  - Read
  - Write
...
Start a new project the right way. This skill thinks like a CEO — it forces product clarity
**before** writing a single file. The output is 16 **pre-filled** project files, not empty
skeletons waiting to be filled.

**Principle:** Every question avad-new asks makes the generated files smarter. If a question
doesn't improve the output, don't ask it.
...
- **Conversation with user:** Always in the user's native/primary language (auto-detected from their first message). Unless the user explicitly requests otherwise.
...
Scan all 16 target files before doing anything else. Show a map and let the user decide.

1. **Scan:** Glob for all 16 files:
...
This phase forces the user to articulate their product thinking before any code exists.
The answers directly feed into generated file content — nothing is wasted.
...
5. **Project type:**
   ...
   API Service → ARCHITECTURE.md with endpoint skeleton).
...
| Check | Trigger | Challenge |
|-------|---------|-----------|
| Vague problem | Problem statement has no specific pain point | "Who has this problem today and how do they solve it without your product?" |
| No clear user | User is too broad ("everyone", "developers") | "Pick ONE person. What's their job title? What frustrates them?" |
| Unrealistic 30-day goal | Goal requires 3+ major systems | "That's ambitious for 30 days. What's the ONE thing that proves the idea works?" |
| Solution-first | Problem describes a feature, not a pain | "That's a solution. What's the problem it solves? Who feels that pain?" |

If all checks pass → "Your product thinking is clear. Moving to tech stack." → proceed to Phase 4.

If any check triggers → ask the challenging question via AskUserQuestion. Loop until clear (max 2 rounds).
After 2 rounds, proceed anyway — don't block forever.
...
Each file skeleton is inline below. The skill substitutes user answers into real content — not
placeholder comments. Content is adapted based on project type preset and detected tech stack.
...
- If a write fails mid-stream (e.g., permission denied, disk full), stop and report which files
  were created vs. which remain. Do not attempt rollback — partial scaffolding is still useful
  and the user can re-run in "fill gaps" mode to complete.
...
- If context7 MCP tools are available: use `resolve-library-id` + `query-docs` to fetch current
  docs for the detected framework (e.g., Next.js app router patterns, FastAPI project structure)
...
5. **30-day Scope Gate** — checks the component list against the user's 30-day goal and flags
   anything that seems too ambitious.
...
<!-- NOTE: The main+dev branch structure below is a recommended default.
     Projects may adjust: solo projects can use main-only, teams may add
     staging/release branches. The key is that protected branches and
     merge strategy are documented. -->
...
Full MIT license text with current year and project name. If user context suggests
proprietary/commercial, use a minimal proprietary stub instead.
...
Stage only files in the `created_files` list (never `git add -A`):
...
Based on user answers, recommend the most valuable next step:

- **If project type is Web App or has UI:** "Your project has a user-facing side. Run `/avad-design-consultation` to create your design system?" → AskUserQuestion with Yes (Recommended) | Skip
- **If problem statement is vague or 30-day goal is ambitious:** "Your PRD could use more depth. Run `/avad-plan-ceo-review` to stress-test your product thinking?" → AskUserQuestion with Yes (Recommended) | Skip
- **If ARCHITECTURE.md was generated with component table:** "Your ARCHITECTURE.md has a solid skeleton. Run `/avad-plan-eng-review` to stress-test the technical decisions?" → AskUserQuestion with Yes | Skip
- **Otherwise:** Print next steps checklist:
  - Flesh out PRD.md (Problem and Users are filled, add Features and Vision)
  - Run `/avad-design-consultation` for design system
  - Run `/avad-plan-eng-review` to validate architecture
  - Run `/avad-plan-ceo-review` to challenge your plan
...
Example: "## **claude Opus 4.6** Project Scaffolding"
