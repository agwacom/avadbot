#!/usr/bin/env bash
# check-freeze.sh — PreToolUse hook for /avad-freeze skill
...
case "$FILE_PATH" in
  ...
  *)
    ...
    # Log hook fire event
    mkdir -p ~/.avadbot/analytics 2>/dev/null || true
    echo '{"event":"hook_fire","skill":"avad-freeze","pattern":"boundary_deny","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}' >> ~/.avadbot/analytics/skill-usage.jsonl 2>/dev/null || true

    printf '{"permissionDecision":"deny","message":"[avad-freeze] Blocked: %s is outside the freeze boundary (%s). Only edits within the frozen directory are allowed."}\n' "$FILE_PATH" "$FREEZE_DIR"
    ;;
esac
