...
description: |
  ...
  visual audits, use /avad-design-review. Use when asked to "review the design plan"
  or "design critique".
...
- If no DESIGN.md: "No design system found. Recommend running /avad-design-consultation first. Proceeding with universal design principles."
...
Re-run loop: invoke /avad-plan-design-review again → re-rate → sections at 8+ get a quick pass, sections below 8 get full treatment.
...
FIX TO 10: If DESIGN.md exists, annotate with specific tokens/components. If no DESIGN.md, flag the gap and recommend `/avad-design-consultation`.
...
If all passes 8+: "Plan is design-complete. Run /avad-design-review after implementation for visual QA."
...
echo '{"skill":"plan-design-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","overall_score":SCORE,"unresolved":UNRESOLVED,"decisions_made":DECISIONS,"commit":"'"$COMMIT"'"}' >> ~/.avadbot/projects/$SLUG/$BRANCH-reviews.jsonl
```

Substitute values from the Completion Summary:
- **STATUS**: "clean" if overall score 8+ AND 0 unresolved; otherwise "issues_open"
- **SCORE**: final overall design score (0-10)
- **UNRESOLVED**: number of unresolved design decisions
- **DECISIONS**: number of design decisions added to the plan
...
**Recommend /avadbot:avad-plan-eng-review if eng review has not been run** — eng review is the required shipping gate. If this design review added significant interaction specifications, new user flows, or changed the information architecture, emphasize that eng review needs to validate the architectural implications.

**Consider recommending /avadbot:avad-plan-ceo-review** — only if this design review revealed fundamental product direction gaps: if the overall design score started below 4/10, if the information architecture had major structural problems, or if the review surfaced questions about whether the right problem is being solved.

**If both are needed, recommend eng review first** (required gate).

Use AskUserQuestion to present the next step. Include only applicable options:
- **A)** Run /avadbot:avad-plan-eng-review next (required gate)
- **B)** Run /avadbot:avad-plan-ceo-review (only if fundamental product gaps found)
