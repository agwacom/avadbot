...
For LLM/prompt changes: if the project has eval suites or prompt-related file patterns documented in CLAUDE.md, check whether this plan touches them. If so, state which eval suites must be run and what baselines to compare against. Then use AskUserQuestion to confirm the eval scope with the user.
...
Example: "## **claude Opus 4.6** Eng Review Response"
