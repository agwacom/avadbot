...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.avadbot'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.avadbot', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.avadbot/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.avadbot');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.avadbot', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.avadbot');
    ...
    test('adds .avadbot/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.avadbot/');
      expect(content).toBe('node_modules/\n.avadbot/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .avadbot/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.avadbot/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.avadbot', 'browse.json') });
  ...
  describe('getRemoteSlug', () => {
    test('returns a non-empty string for current repo', () => {
      const slug = getRemoteSlug();
      // Returns owner-repo if remote exists, or directory basename as fallback
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:example-org/example-repo.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('example-org-example-repo');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/example-org/example-repo.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('example-org-example-repo');
    });
