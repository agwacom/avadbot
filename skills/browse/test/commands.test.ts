...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/avadbot-cli-');
    const execPath = path.join(root, '.claude/skills/avadbot/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/avadbot/browse/src/server.ts');
