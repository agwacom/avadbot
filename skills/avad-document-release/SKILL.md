...
You are running the `/avad-document-release` workflow. This runs **after `/avad-ship`** (code committed, PR
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.avadbot/*" -not -path "./.context/*" | sort
```
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/avad-ship` from the
...
This is a second pass that complements `/avad-ship`'s Step 5.5. Read `review/TODOS-format.md` (if
...
```bash
gh pr view --json body -q .body > /tmp/avadbot-pr-body-$$.md
```
...
3. The Documentation section should include a **doc diff preview** — for each file modified,
   describe what specifically changed (e.g., "README.md: added /avad-document-release to skills
   table, updated skill count from 9 to 10").

4. Write the updated body back:

```bash
gh pr edit --body-file /tmp/avadbot-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/avadbot-pr-body-$$.md
```
...
- Already bumped — version was set by /avad-ship
