...
## Arguments
- `/avad-retro` — default: last 7 days
- `/avad-retro 24h` — last 24 hours
- `/avad-retro 14d` — last 14 days
- `/avad-retro 30d` — last 30 days
- `/avad-retro compare` — compare current window vs prior same-length window
...
5. Save only the current-window snapshot to `.context/avad-retros/` (same as a normal retro run); do **not** persist the prior-window metrics.
...
- Output directly to the conversation — do NOT write to filesystem (except the `.context/avad-retros/` JSON snapshot)

## Important Rules

- ALL narrative output goes directly to the user in the conversation. The ONLY file written is the `.context/avad-retros/` JSON snapshot.
...
2. **Labels:**
   - Review role label: `retro-review`
   ...
   - Create the label if it does not exist (color: `#C5DEF5`, description: "Engineering retrospective").
...
Example: "## **claude Opus 4.6** Retro Review Response"
