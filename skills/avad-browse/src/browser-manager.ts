...
export class BrowserManager {
  ...
  private buildContextOptions(): BrowserContextOptions {
    ...
    if (this.customUserAgent) {
      opts.userAgent = this.customUserAgent;
    }
    return opts;
  }
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .avadbot/browse-*.log');
      process.exit(1);
    });
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      const newContext = await newBrowser.newContext(this.buildContextOptions());
      ...
      // Save all old references so we can roll back if restoreState fails
      ...
      // Restore failed — close the new browser, restore old state
   ...
   * Resyncs tab map from the browser context (user may have opened/closed tabs),
   * clears stale refs. The meta-command handler calls handleSnapshot() after this.
   */
  async resume(): Promise<void> {
    if (!this.context) return;

    // Resync pages map: user may have opened or closed tabs during handoff
    const livePages = this.context.pages();
    const oldPages = new Set(this.pages.values());

    // Remove tabs the user closed
    for (const [id, page] of this.pages) {
      if (!livePages.includes(page) || page.isClosed()) {
        this.pages.delete(id);
      }
    }

    // Add tabs the user opened
    for (const page of livePages) {
      if (!oldPages.has(page) && !page.isClosed()) {
        const id = this.nextTabId++;
        this.pages.set(id, page);
        this.wirePageEvents(page);
      }
    ...
    if (!this.pages.has(this.activeTabId) && this.pages.size > 0) {
      this.activeTabId = [...this.pages.keys()][this.pages.size - 1];
    }
