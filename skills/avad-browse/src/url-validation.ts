...
const BLOCKED_METADATA_HOSTS = new Set([
  ...
  'fd00:ec2::254',    // AWS IMDS IPv6 endpoint (Nitro instances)
...
function isMetadataIp(hostname: string): boolean {
  ...
  // Check IPv4-mapped IPv6 forms (::ffff:169.254.169.254 → ::ffff:a9fe:a9fe)
  const lower = hostname.toLowerCase().replace(/^\[|\]$/g, '');
  if (lower.startsWith('::ffff:')) {
    const mapped = lower.slice(7); // strip ::ffff: prefix
    // Could be dotted-quad (::ffff:169.254.169.254) or hex (::ffff:a9fe:a9fe)
    if (BLOCKED_METADATA_HOSTS.has(mapped)) return true;
    // Normalize dotted-quad form through URL constructor
    try {
      const probe = new URL(`http://${mapped}`);
      if (BLOCKED_METADATA_HOSTS.has(probe.hostname)) return true;
    } catch { /* not parseable */ }
    // Check hex form: a9fe:a9fe = 169.254.169.254
    if (mapped === 'a9fe:a9fe') return true;
  }
