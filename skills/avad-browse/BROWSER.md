...
This document covers the command reference and internals of avadbot's headless browser.
...
avadbot's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
└── dist/
    └── avad-browse         # Compiled binary (~58MB, Bun --compile)
```
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.avadbot/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.avadbot/browse-console.log`
- Network: `.avadbot/browse-network.log`
- Dialog: `.avadbot/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.avadbot/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.avadbot/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.avadbot/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.avadbot/browse.json` | Path to state file (CLI passes to server) |
...
| **avadbot browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. avadbot burns zero.
...
avadbot skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
bun run build            # compile to avad-browse/dist/avad-browse
```

### Dev mode vs compiled binary

During development, use `bun run dev` instead of the compiled binary. It runs `avad-browse/src/cli.ts` directly with Bun, so you get instant feedback without a compile step:
...
The compiled binary (`bun run build`) is only needed for distribution. It produces a single ~58MB executable at `avad-browse/dist/avad-browse` using Bun's `--compile` flag.

### Running tests

```bash
bun test                         # run all tests
bun test avad-browse/test/commands              # run command integration tests only
bun test avad-browse/test/snapshot              # run snapshot tests only
bun test avad-browse/test/cookie-import-browser # run cookie import unit tests only
```

Tests spin up a local HTTP server (`avad-browse/test/test-server.ts`) serving HTML fixtures from `avad-browse/test/fixtures/`, then exercise the CLI commands against those pages. 203 tests across 3 files, ~15 seconds total.

### Source map

| File | Role |
|------|------|
| `avad-browse/src/cli.ts` | Entry point. Reads `.avadbot/browse.json`, sends HTTP to the server, prints response. |
| `avad-browse/src/server.ts` | Bun HTTP server. Routes commands to the right handler. Manages idle timeout. |
| `avad-browse/src/browser-manager.ts` | Chromium lifecycle — launch, tab management, ref map, crash detection. |
| `avad-browse/src/snapshot.ts` | Parses accessibility tree, assigns `@e`/`@c` refs, builds Locator map. Handles `--diff`, `--annotate`, `-C`. |
| `avad-browse/src/read-commands.ts` | Non-mutating commands: `text`, `html`, `links`, `js`, `css`, `is`, `dialog`, `forms`, etc. Exports `getCleanText()`. |
| `avad-browse/src/write-commands.ts` | Mutating commands: `goto`, `click`, `fill`, `upload`, `dialog-accept`, `useragent` (with context recreation), etc. |
| `avad-browse/src/meta-commands.ts` | Server management, chain routing, diff (DRY via `getCleanText`), snapshot delegation. |
| `avad-browse/src/cookie-import-browser.ts` | Decrypt Chromium cookies via macOS Keychain + PBKDF2/AES-128-CBC. Auto-detects installed browsers. |
| `avad-browse/src/cookie-picker-routes.ts` | HTTP routes for `/cookie-picker/*` — browser list, domain search, import, remove. |
| `avad-browse/src/cookie-picker-ui.ts` | Self-contained HTML generator for the interactive cookie picker (dark theme, no frameworks). |
| `avad-browse/src/buffers.ts` | `CircularBuffer<T>` (O(1) ring buffer) + console/network/dialog capture with async disk flush. |

### Deploying changes

- **Dev mode** (recommended): `bin/dev-setup` — symlinks skills, changes are live. See [CONTRIBUTING.md](../../CONTRIBUTING.md).
- **Production**: `./setup` — copies skills to `~/.claude/skills/`, builds binary if needed.
- **Plugin mode**: `claude plugin update avadbot@avadbot-local` then restart session.

### Adding a new command

1. Add the handler in `read-commands.ts` (non-mutating) or `write-commands.ts` (mutating)
2. Register the route in `server.ts`
3. Add a test case in `avad-browse/test/commands.test.ts` with an HTML fixture if needed
