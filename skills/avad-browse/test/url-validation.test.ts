...
describe('validateNavigationUrl', () => {
  ...
  it('blocks AWS IMDS IPv6 endpoint (fd00:ec2::254)', () => {
    expect(() => validateNavigationUrl('http://[fd00:ec2::254]/latest/meta-data/')).toThrow(/cloud metadata/i);
  });

  it('blocks IPv4-mapped IPv6 form of metadata IP', () => {
    expect(() => validateNavigationUrl('http://[::ffff:169.254.169.254]/')).toThrow(/cloud metadata/i);
  });

  it('blocks IPv4-mapped IPv6 hex form (::ffff:a9fe:a9fe)', () => {
    expect(() => validateNavigationUrl('http://[::ffff:a9fe:a9fe]/')).toThrow(/cloud metadata/i);
  });
